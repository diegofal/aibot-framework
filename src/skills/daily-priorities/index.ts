...
> = {
  async daily_set_priorities(args, ctx) {
    ...
    if (priorities.length > 5) {
      return {
        success: false,
        message: 'Maximum 5 priorities allowed. Focus on what matters most.',
      };
    }
  ...
  async daily_review(args, ctx) {
    ...
    if (!entry) {
      return {
        success: false,
        message: 'No priorities set for today. Set priorities first with /priorities set',
      };
    }
    ...
    return {
      success: true,
      entry,
      summary: { completed, total, percent: Math.round((completed / total) * 100) },
    };
  },
  ...
  async daily_get_history(args, ctx) {
    ...
    const entries = dates.filter((date) => data.entries[date]).map((date) => data.entries[date]);
  ...
  async daily_get_stats(args, ctx) {
    ...
    const completionRate =
      totalPriorities > 0 ? Math.round((totalCompleted / totalPriorities) * 100) : 0;
  ...
  commands: {
    priorities: {
      ...
      async handler(args: string[], ctx) {
        ...
        if (!subcommand) {
          const result = (await handlers.daily_get_priorities({}, ctx)) as {
            success: boolean;
...
/priorities set "First priority" "Second priority" "Third priority"`;
          ...
          if (result.entry.reflection) {
            lines.push(
              '',
              `📝 Evening review completed at ${new Date(result.entry.reviewedAt!).toLocaleTimeString()}`
            );
          }
        ...
        if (subcommand === 'set') {
          ...
          const result = (await handlers.daily_set_priorities({ priorities }, ctx)) as {
            success: boolean;
            entry?: DailyEntry;
            message?: string;
          };
        ...
        if (subcommand === 'check' || subcommand === 'done' || subcommand === 'complete') {
          const index = Number.parseInt(rest.split(' ')[0]) - 1;
          if (Number.isNaN(index)) {
            return 'Usage: /priorities check <number> (e.g., /priorities check 1)';
          }

          const result = (await handlers.daily_check_progress({ index }, ctx)) as {
            success: boolean;
            checked?: PriorityItem;
            message?: string;
          };
          if (!result.success) {
            return `❌ ${result.message}`;
        ...
        if (subcommand === 'uncheck') {
          ...
          entry.priorities[index].completedAt = undefined;
          saveEntry(ctx, entry);
        ...
        if (subcommand === 'review') {
          ...
          const result = (await handlers.daily_review({ reflection: rest }, ctx)) as {
            success: boolean;
            entry?: DailyEntry;
        ...
        if (subcommand === 'history') {
          const days = Number.parseInt(rest) || 7;
          const result = (await handlers.daily_get_history({ days }, ctx)) as {
            success: boolean;
            entries: DailyEntry[];
            count: number;
          };
          ...
          const lines = [`📊 Last ${result.count} days:`, ''];
        ...
        // Stats
        if (subcommand === 'stats') {
          const days = Number.parseInt(rest) || 30;
          const result = (await handlers.daily_get_stats({ days }, ctx)) as {
            success: boolean;
            stats: {
              daysTracked: number;

...
function parseTime(when: string): Date | null {
  ...
  if (tomorrowMatch) {
    ...
    if (tomorrowMatch[1]) {
      result.setHours(parseInt(tomorrowMatch[1]), parseInt(tomorrowMatch[2] || '0'), 0, 0);
    } else {
  ...
  if (todayMatch) {
    const result = new Date(now);
    if (todayMatch[1]) {
      result.setHours(parseInt(todayMatch[1]), parseInt(todayMatch[2] || '0'), 0, 0);
    }
    return result;
  }
...
export const handlers: Record<string, (args: Record<string, unknown>, context: SkillContext) => Promise<unknown>> = {
  async reminders_set(args, ctx) {
    try {
      ...
      if (!scheduledAt) {
        return { success: false, message: `Could not parse time: "${when}". Try "in 5 minutes", "tomorrow at 9am", "at 3pm", or "2026-02-26 14:00"` };
      }
      ...
      // Schedule the job
      const schedule: { kind: 'at'; at: string } | { kind: 'cron'; expr: string; tz: string } = recurring && recurringPattern
        ? { kind: 'cron', expr: recurringPattern, tz: Intl.DateTimeFormat().resolvedOptions().timeZone }
  ...
  async reminders_snooze(args, ctx) {
    try {
      ...
      if (!snoozeUntil) {
        return { success: false, message: `Could not parse duration: "${duration}". Try "5 minutes", "30 minutes", "1 hour", "tomorrow"` };
      }
...
const skill: Skill = {
  ...
  commands: {
    remind: {
      description: 'Reminder commands: set <text> in/at <time>, list, delete <id>, snooze <id> <duration>, done <id>',

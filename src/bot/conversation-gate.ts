...
export class ConversationGate {
  ...
  async evaluate(
    grammyCtx: Context & { message: { text: string; message_id: number; reply_to_message?: { message_id: number } }; chat: { id: number; type: string }; from: { id: number; username?: string; first_name?: string } },
    config: BotConfig,
    botLogger: Logger,
    ...
    if (senderAgent) {
      ...
      const check = this.ctx.collaborationTracker.checkAndRecord(
        senderAgent.botId, config.id, chatId,
      );
      if (!check.allowed) {
        botLogger.info(
    ...
    const botUsername = grammyCtx.me?.username;
    const chatTitle = 'title' in grammyCtx.chat ? (grammyCtx.chat as { title?: string }).title : undefined;

    // 6. Group activation gate
    if (isGroup && sessionConfig.enabled && !isPeerBotMessage) {
      const gateResult = await this.evaluateGroupGate(grammyCtx, config, botLogger, botUsername, chatTitle);
    ...
    if (isGroup && botUsername) {
      strippedText = this.ctx.sessionManager.stripBotMention(
        text, botUsername, config.mentionPatterns,
      );
    }
  ...
  private async evaluateGroupGate(
    grammyCtx: Context & { message: { text: string }; chat: { id: number; type: string }; from: { id: number; username?: string; first_name?: string } },
    config: BotConfig,
    botLogger: Logger,
    ...
    let groupReason = this.ctx.sessionManager.shouldRespondInGroup(
      grammyCtx, botUsername, config.id, config.mentionPatterns,
    );
    ...
    if (groupReason === 'replyWindow' && sessionConfig.llmRelevanceCheck.enabled) {
      ...
      const isRelevant = await this.groupActivation.checkLlmRelevance(
        grammyCtx, config.name, serializedKey, config.id,
      );
      if (!isRelevant) {
        botLogger.info(
          {
            chatId, chatTitle, userId: grammyCtx.from?.id,
    ...
    if (
      ...
      const shouldRespond = await this.groupActivation.checkBroadcastRelevance(
        grammyCtx, config.name, config.id,
      );
      if (shouldRespond) {
      ...
      botLogger.info(
        {
          ...
          firstName: grammyCtx.from?.first_name,
          text: grammyCtx.message.text.substring(0, 80),
          botUsername, mentionPatterns: config.mentionPatterns,
        },

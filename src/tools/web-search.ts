  ...
  return {
    ...
    async execute(args: Record<string, unknown>, logger: Logger): Promise<ToolResult> {
      ...
      try {
        ...
        if (results.length === 0) {
          const content = wrapExternalContent(`No results found for: "${query}"`);
        ...
        const content = wrapExternalContent(`Search results for "${query}":\n\n${formatted}`);

        cache.set(query, content);
        logger.debug({ query, resultCount: results.length }, 'web_search completed');

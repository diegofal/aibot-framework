  ...
  while (remaining.size > 0) {
    ...
    for (const candidate of remaining) {
      ...
      if (
        ...
        (mmrScore === bestMMRScore &&
          candidate.score > (bestItem?.score ?? Number.NEGATIVE_INFINITY))
      ) {
...
/**
 * Apply MMR re-ranking to memory search results.
 * Adapts the generic MMR function to work with the MemorySearchResult format.
 */
export function applyMMRToMemoryResults<T extends MemorySearchResult>(
  results: T[],

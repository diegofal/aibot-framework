...
- **Claude CLI startup preflight** (`src/bot/claude-cli-preflight.ts`, `src/index.ts`, `tests/claude-cli-preflight.test.ts`): `checkClaudeCli()` reports whether the binary is on PATH, its version, and whether the config dir holds a login; `formatPreflight()` renders the boot line. Wired non-fatally into `main()` — `info` when healthy, `warn` only when the claude-cli backend is actually in the failover chain *and* the binary or credentials are missing, so an Ollama-only deployment gets no noise. This matters because an unauthenticated CLI does not fail loudly: it exits 0 with `"result": "Not logged in · Please run /login"`, which `claudeGenerate` would otherwise hand back as if it were the model's answer. Every dependency (`which`, `run`, `exists`) is injectable, mirroring `resolveClaudeBin`'s `WhichFn`, so the 13 tests stay hermetic. A missing credentials file on darwin reports `unknown` rather than `missing`, since macOS keeps the login in the Keychain.

...
### Fixed
- **Claude CLI native tool bypass for blocked permissions**: When a framework tool (e.g. `exec`) was blocked via `toolPermissions`, the bot could still execute the equivalent action through Claude CLI's built-in native tools (e.g. `Bash`). Now blocked framework tools are mapped to their Claude CLI native equivalents and passed via `--disallowedTools` to prevent bypass. Mapping: `exec`→`Bash`, `file_write`→`Write`, `file_edit`→`Edit`, `file_read`→`Read`, `browser`→`WebFetch,WebSearch`. New `NATIVE_TOOL_MAP` constant and `getBlockedNativeTools()` in `tool-permissions.ts`. Wired through `ChatOptions.blockedNativeTools` → `ClaudeCliLLMClient` → `claudeGenerateWithTools()`. Also fixed agent-loop not passing `permissionMode: 'agent-loop'` to `getDefinitionsForBot()`. Tests: 9 new tests in `tests/tool-permissions.test.ts`.

...
describe('AskHumanStore', () => {
  ...
  test('getAll() returns pending questions with correct fields', () => {
    store.ask('bot1', 123, 'What should I do?');
    store.ask('bot2', 456, 'Pick a strategy');
  ...
  test('dispose clears all pending questions', async () => {
    const { promise: p1 } = store.ask('bot1', 100, 'q1');
    const { promise: p2 } = store.ask('bot2', 200, 'q2');
  ...
  test('answerById stores answer in answered map', () => {
    const { id } = store.ask('bot1', 100, 'What strategy?');
  ...
  test('consumeAnswersForBot returns and deletes answers', () => {
    const { id: id1 } = store.ask('bot1', 100, 'q1');
    const { id: id2 } = store.ask('bot1', 100, 'q2');
  ...
  test('consumeAnswersForBot returns empty array for unknown bot', () => {
    const { id } = store.ask('bot1', 100, 'q1');
  ...
  test('handleReply also stores in answered map', () => {
    const { id } = store.ask('bot1', 100, 'What next?');
  ...
  test('onDismiss callback is called on dismiss', () => {
    ...
    const { id, promise } = dismissStore.ask('bot1', 100, 'q1');
    promise.catch(() => {});
